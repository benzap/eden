...
* Programming in Eden
  This is a short manual explaining how to use the Eden programming language.
** Values and Types
...
** Variables
   Variables in Eden are presented in the form of Symbols.

   #+BEGIN_SRC
   x y foo bar is-value? set-value!
   #+END_SRC
...
*** For Statement
    ...
    The second type of ~for~ statement, called the ~for-each~
...
**** ~json~ Library
     ...
     - json.parse(s) :: provided by cheshire.core/parse-string
     - json.stringify(coll) :: provided by cheshire.core/generate-string
**** ~edn~ Library
     EDN library parser provided by [[https://github.com/clojure/tools.reader][tools.reader]]
     - edn.parse(s) :: provided by clojure.tools.reader.edn/read-string
     - edn.stringify(coll) :: clojure.core/pr-str
**** ~transit~ Library
     Transit Reader and Generator provided by [[https://github.com/cognitect/transit-clj][transit-clj]]

     *incomplete*

     - transit.read :: provided by cognitect.transit/read
     - transit.write :: provided by cognitect.transit/write
*** ~http~ Library
    HTTP Client and Server provided by [[http://www.http-kit.org/][http-kit]]
    HTTP Router provided by [[https://github.com/juxt/bidi][bidi]]
    
    - http.router.make-handler :: provided by bidi.ring/make-handler
    - http.server.run-server :: provided by org.httpkit.server/run-server
    - http.client.get :: provided by org.httpkit.client/get
...
*** ~shell~ Library
    ...
    to reflection issues that will be resolved in future native executables
*** ~operator~ Library
    Includes all of the clojure equivalent operators. Useful for
    additional performance in certain applications
    - operator.add :: clojure.core/+
    - operator.sub :: clojure.core/-
    - operator.mult :: clojure.core/*
    - operator.div :: clojure.core//
    - operator.not :: clojure.core/not
    - operator.and :: clojure.core/and
    - operator.or :: clojure.core/or
